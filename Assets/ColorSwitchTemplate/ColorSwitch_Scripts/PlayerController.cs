...
public class PlayerController : MonoBehaviour
{
    ...
    private Renderer playerRenderer;                    //Player's Sphere Mesh Renderer(originally had different mesh shapes, and the player rotated)
    ...
    private float localSoundVolume;                     //local sound volume was multiplied by the TemporaryGameVars.soundVolume(done to tweak fx/music balance without mixer)
    public GameObject deathParticles, flashObject;      //the particles systems that are instantiated when player collides/dies
    private Camera mainCam;                             //reference to main scene cam(used to GetComponet the Camera Shake and Chromatic Abberation Scripts)
    ...
    void Awake()
    {
        ...
        //Disable initially.. we didn't lose yet people!
...
//        if (Input.GetMouseButtonDown(0))
    ...
    /// <summary>
    /// This Method is very similar to PlayerDeathEffectsOnly, except that it reloads the level.  In earlier designs the level just reloaded when you died, and then 
    /// later the endGameCanvas was added.
    /// </summary>
